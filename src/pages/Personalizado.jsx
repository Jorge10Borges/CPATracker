      ...
      toolbar={
        <>
          <div className="flex items-center gap-2 text-sm">
            <label htmlFor="token-personalizado" className="font-semibold">Personalizado:</label>
            <select
              id="token-personalizado"
          ...
          <tbody>
            {table.getRowModel().rows.map(row => (
              ...
              >
                {row.getVisibleCells().map(cell => {
                  const isLeft = cell.column.id === 'nombre';
                  return (
                    <td
                      key={cell.id}
            ...
            <tr className="bg-gray-100 font-bold">
              {table.getAllColumns().map((col, idx) => {
                ...
                if (id === 'nombre') return <td key={id} className="px-4 py-2 border-t text-left">Total</td>;
